...
export default async function ExercisePage({
  ...
  const rows = await prisma.workoutEntry.findMany({
    where: isClimbView
      ? { userId: session.userId, gym: { equals: gym, mode: "insensitive" }, grade }
      : { userId: session.userId, exerciseName: { equals: name, mode: "insensitive" } },
  ...
  // Manual records (backfilled PRs with no logged entry, see /records) can beat every entry here
  // -- fetch the ones for this exercise so the PR badge reflects the true best, not just the best
  // among the entries that happen to have video.
  const manualRows = isClimbView
    ? []
    : await prisma.manualRecord.findMany({
        where: { userId: session.userId, exerciseName: { equals: name, mode: "insensitive" } },
      });
