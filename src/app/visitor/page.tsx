...
import { Dumbbell, ClipboardList, Clock, Flame } from "lucide-react";
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-16">
        <EmptyState
          ...
          description="This training log doesn't have any entries logged yet."
        />
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-6xl space-y-10 px-4 py-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">Here&apos;s how training has been going.</p>
        </div>
      </div>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {thisWeekTracked.map((e) => (
              <EntryCard key={e.id} entry={e} basePath="/visitor" />
            ))}
          </div>
        </section>
      )}
